...
def get_queries_from_db():
    try:
        ...
        print(f"[DEBUG] {len(queries)} 件のクエリを取得", flush=True)
        ...
        print(f"[DB ERROR] クエリ取得中にエラー: {e}", flush=True)
        traceback.print_exc()
        return []
...
def get_search_query_data(site_url, keyword):
    ...
    try:
        ...
        print(f'Error retrieving data for keyword {keyword}: {e}', flush=True)
        return [], keyword

def insert_gsc_data(query_keyword, total_impressions, total_clicks, avg_ctr, avg_position, start_date, end_date):
    try:
        print(f"[DEBUG] Insert: {query_keyword}, Impr={total_impressions}, Clicks={total_clicks}, CTR={avg_ctr}, Pos={avg_position}", flush=True)
    ...
    for start_date, end_date in date_ranges:
        ...
        for keyword in queries:
            ...
            if record_exists(keyword, db_start_date, db_end_date):
                print(f"[SKIP] 既に登録済み: {keyword}, {db_start_date} - {db_end_date}", flush=True)
                continue
            ...
            print(f'URL: {original_query}, 合計表示回数: {total_impressions}, 合計クリック数: {total_clicks}, 平均CTR: {avg_ctr:.4f}, 平均掲載順位: {avg_position:.2f}', flush=True)
