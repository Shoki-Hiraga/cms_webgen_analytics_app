<x-app-layout>
    <x-slot name="header">
        <x-slot name="title">編集 | @include('components.Gsc_Media_Urls') </x-slot>
        <h2 class="page-title">@include('components.Gsc_Media_Urls') 編集</h2>
    </x-slot>

    <div class="form-container">
        <form action="{{ route('admin.gsc_media_url_listurls.update', $url->id) }}" method="POST">
            @csrf
            @method('PUT')
        ...
        <form action="{{ route('admin.gsc_media_url_listurls.destroy', $url->id) }}" method="POST" style="margin-top: 20px;">
            @csrf
            @method('DELETE')
