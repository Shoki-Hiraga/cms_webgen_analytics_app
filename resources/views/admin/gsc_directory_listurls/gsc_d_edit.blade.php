<x-app-layout>
    ...
    <div class="form-container">
        <form action="{{ route('admin.gsc_directory_listurls.update', $url->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <a href="{{ route('admin.gsc_directory_listurls.index') }}" class="btn-secondary">キャンセル</a>
        </form>

        <form action="{{ route('admin.gsc_directory_listurls.destroy', $url->id) }}" method="POST" style="margin-top: 20px;">
            @csrf
            @method('DELETE')
